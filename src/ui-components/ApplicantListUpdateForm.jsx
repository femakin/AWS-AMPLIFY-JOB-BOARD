...
export default function ApplicantListUpdateForm(props) {
  const {
    id: idProp,
    applicantList,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = applicantListRecord
      ? { ...initialValues, ...applicantListRecord }
      : initialValues;
    setName(cleanValues.Name);
    setEmail(cleanValues.Email);
    setMessage(cleanValues.Message);
    setPortfolioLink(cleanValues.PortfolioLink);
    setStatus(cleanValues.Status);
    setJobID(cleanValues.JobID);
    setErrors({});
  };
  const [applicantListRecord, setApplicantListRecord] =
    React.useState(applicantList);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp
        ? await DataStore.query(ApplicantList, idProp)
        : applicantList;
      setApplicantListRecord(record);
    };
    queryData();
  }, [idProp, applicantList]);
  React.useEffect(resetStateValues, [applicantListRecord]);
  const validations = {
    Name: [],
    Email: [{ type: "Email" }],
    Message: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            ApplicantList.copyOf(applicantListRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "ApplicantListUpdateForm")}
      {...rest}
    >
      <TextField
        label="Name"
      ...
      >
        <Button
          ...
          isDisabled={!(idProp || applicantList)}
