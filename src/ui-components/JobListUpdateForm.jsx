...
export default function JobListUpdateForm(props) {
  const {
    id: idProp,
    jobList,
    onSuccess,
  ...
  const resetStateValues = () => {
    const cleanValues = jobListRecord
      ? { ...initialValues, ...jobListRecord }
      : initialValues;
  ...
  const [jobListRecord, setJobListRecord] = React.useState(jobList);
  React.useEffect(() => {
    const queryData = async () => {
      const record = idProp ? await DataStore.query(JobList, idProp) : jobList;
      setJobListRecord(record);
    };
    queryData();
  }, [idProp, jobList]);
  React.useEffect(resetStateValues, [jobListRecord]);
  const validations = {
    JobPosition: [],
    Category: [],
    Location: [],
  ...
  return (
    <Grid
      ...
      onSubmit={async (event) => {
        ...
        try {
          ...
          await DataStore.save(
            JobList.copyOf(jobListRecord, (updated) => {
      ...
      {...getOverrideProps(overrides, "JobListUpdateForm")}
      {...rest}
    >
      <TextField
        label="Job position"
